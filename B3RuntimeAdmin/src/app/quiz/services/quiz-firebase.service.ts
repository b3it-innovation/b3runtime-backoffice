...
import { Track } from '../../map/model/track.model';
import { MapError } from '../../map/model/map-error.model';
...
export class QuizFirebaseService {
    ...
    getQuestions(): Observable<Question[] | QuizError> {
     return   firebaseObservable(firebase.database().ref('questions'), 'once')
            ...
            .catch((error: firebase.FirebaseError) => Observable.of(this.qs));
    }
    ...
    editQuestion(question: Question): Observable<Question | QuizError> {
        ...
        // return Observable.fromPromise(firebase.database().ref('questions').child(question.key)
        //     .set(this.mapFromQuestionModelToQuestionFirebaseModel(question))
        //     .then(() => {
        //         question.categories.forEach((category) => {
        //             console.log('1');
        //             firebase.database().ref('/categories/' + category + '/questions/').once('value', function (snapshot) {
        //                 snapshot.ref.update({ [question.key]: true });
        //             });
        //         })
        //     }))
        //     .switchMap(() => { console.log('2ssss'); return Observable.of(question) })
