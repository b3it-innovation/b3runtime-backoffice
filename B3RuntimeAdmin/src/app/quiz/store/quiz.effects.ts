...
import { AddQuestionRequest, SelectQuestionRequest, DragQuestionRequest } from '../payload/question';
...
export class QuizEffects {
  ...
  @Effect()
  selectQuestion$: Observable<Action> = this.actions$
    .ofType(QuizActions.Types.SELECT_QUESTION)
    .map(toPayload => {
      return this.quizActions.selectQuestionSuccessAction()
    })
    .catch((error: QuizError) => {
      error.action = QuizActions.Types.SELECT_QUESTION;
      return Observable.of(this.quizActions.quizErrorAction(error));
    });

  @Effect()
  editQuestion$: Observable<Action> = this.actions$
    .ofType(QuizActions.Types.EDIT_QUESTION)
    .map(toPayload)
    .switchMap((data: AddQuestionRequest) => this.quizService.editQuestion(data.question)
      .map((response) => this.quizActions.editQuestionSuccessAction({ question: data.question }))
      .catch((error: QuizError) => {
        error.action = QuizActions.Types.ADD_CATEGORY;
        return Observable.of(this.quizActions.quizErrorAction(error));
      }));
  ...
  @Effect()
  dragQuestion$: Observable<Action> = this.actions$
    .ofType(QuizActions.Types.DRAG_QUESTION)
    .map((toPayload))
    .switchMap((data: DragQuestionRequest) => Observable.of(data.question)
      .map((response) => this.quizActions.dropQuestionSuccessAction({ question: data.question }))
      .catch((error: QuizError) => {
        error.action = QuizActions.Types.ADD_CATEGORY;
        return Observable.of(this.quizActions.quizErrorAction(error));
      }));
